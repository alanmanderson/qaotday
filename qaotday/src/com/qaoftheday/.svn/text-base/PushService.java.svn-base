...
public class PushService extends Service
{
	...
	private static final String		MQTT_HOST = "72.167.167.167";//"209.124.50.185";
	...
	// Notification title
	public static String			NOTIF_TITLE = "Q&A of the Day"; 	
	// Notification id
	...
	private synchronized void start() {
		...
		if (mStarted == true) {
			...
			mConnection.unsubscribeFromAll();
			try {
	...
	private void showNotification(String text) {
		...
		n.icon = com.qaoftheday.R.drawable.icon;
		n.when = System.currentTimeMillis();

		// Simply open the parent activity
		Intent questionIntent = new Intent(this, QuestionActivity.class);
		questionIntent.addCategory("test.foobar.123");
		questionIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		questionIntent.putExtra("quest",text);
  		questionIntent.putExtra("devID",mPrefs.getString(PREF_DEVICE_ID, null));
		PendingIntent pi = PendingIntent.getActivity(this, 0,
				questionIntent, 0);
		  //new Intent(this, PushActivity.class), 0);
		// Change the name of the notification here
		n.setLatestEventInfo(this, NOTIF_TITLE, text.split(":")[1], pi);
	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		public MQTTConnection(String brokerHostName, String initTopic) throws MqttException {
	    	...
	    	String mqttConnSpec = "tcp://" + brokerHostName + "@" + MQTT_BROKER_PORT_NUM;
	    	    ...
	    	    //Emulator Device ID = 9774d56d682e549c
		...
		private void unsubscribeToTopic(String topicName) throws MqttNotConnectedException, IllegalArgumentException, MqttException {
			if ((mqttClient == null) || (mqttClient.isConnected() == false)) {
				log("Connection error" + " No connection");
			} else {
				String[] topics = { topicName };
		...
		private void unsubscribeFromAll(){
			if (mRegedTopics == null) return;
			for (String topic: mRegedTopics){
				try {
					mqttClient.unsubscribe(new String[] { MQTT_CLIENT_ID + "/" + topic });
				} catch (MqttNotConnectedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				} catch (IllegalArgumentException e) {
		...
		private void subscribeToTopics() throws MqttException {
			...
			if ((mqttClient == null) || (mqttClient.isConnected() == false)) {
				...
				SharedPreferences settings = getSharedPreferences(getString(R.string.pref_file),0);
					...
					strTopics = strTopics.replace("]", "");
					mRegedTopics = strTopics.split(", ");
					
					for(int i=0; i<mRegedTopics.length; i++){
						String[] newTopic = { MQTT_CLIENT_ID + "/" + mRegedTopics[i] };
						mqttClient.subscribe(newTopic, MQTT_QUALITIES_OF_SERVICE);
					}
					//mqttClient.subscribe(topics, MQTT_QUALITIES_OF_SERVICE);
		  	  	}
			}
		...
		public void publishArrived(String topicName, byte[] payload, int qos, boolean retained) {
			...
			showNotification(s);
			log("Got message: " + s);
			Intent questionIntent = new Intent(Intent.ACTION_VIEW);
			questionIntent.addCategory("test.foobar.123");
			questionIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			questionIntent.putExtra("quest",s);
	  		questionIntent.putExtra("devID",mPrefs.getString(PREF_DEVICE_ID, null));
